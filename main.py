...
BOT_TOKEN = "8472312163:AAGonM3rpadQYOJt2TszDWD4QLaTDi57vao"
ADMIN_ID = 8458643145
ADMINS = [8458643145]
...
save_counter = 0  # for periodic save
    ...
    try:
        if os.path.exists(SEEN_FILE):
            with open(SEEN_FILE, 'r') as f:
                data = json.load(f)
                seen_otps = set(data)
            logger.info(f"Loaded {len(seen_otps)} seen OTPs from {SEEN_FILE}")
        else:
            logger.info("No seen OTPs file found, starting fresh")
...
def save_seen_otps():
    try:
        ...
        logger.debug(f"Saved {len(seen_otps)} seen OTPs to {SEEN_FILE}")
    except Exception as e:
...
GROUP_KEYBOARD = InlineKeyboardMarkup([
    [
        InlineKeyboardButton("📱 Channel", url="https://t.me/AyanwithEarningsYT"),
        InlineKeyboardButton("☎️ Number", url="https://t.me/Ayannumber1"),
    ],
    [
        InlineKeyboardButton("💻 DEVELOPER", url="https://t.me/Ayanpak5"),
        InlineKeyboardButton("💬 Main Chat", url="https://t.me/AyanOTPDISCUSSION"),
    ],
])
        ...
        while True:
            try:
                ...
                if not initialized:
                    ...
                    save_seen_otps()  # save after initial load
                else:
                    for otp in otps:
                        ...
                        # Periodically save seen_otps (every 100 new OTPs)
                        if save_counter >= 100:
                            save_seen_otps()
                            save_counter = 0
                ...
                if len(seen_otps) > 5000:
                    ...
                    for k in excess:
                        seen_otps.discard(k)
                    save_seen_otps()  # save after trimming
...
async def callback_handler(update: Update, context):
    ...
    if data == "check_join":
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("\U0001f4f1 OTP Group", url="https://t.me/tempnumberotp7")],
        ])
...
async def post_init(application):
    load_seen_otps()  # load saved OTPs
